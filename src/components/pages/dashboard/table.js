...
import { URL, URL_CARS} from '../../../data/constants';
...
export default function TableDashboard() {
  ...
  // const [keyType, setKeyType] = useState('toDayOrders');
  ...
  return (
    <div className='h-[70vh] container'>
                ...
                <Fragment key={i}>
                  <tr
                    ...
                    {dashboardTableOrdersData.map((row, j) =>
                      (<td key={j} className="px-6 py-4">{row === 'tenant_name' ? car[row]['name'] && car[row]['name'] : car[row]}</td>)
                    )}
                  </tr>
                </Fragment>
              );

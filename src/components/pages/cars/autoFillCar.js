...
const AutoFillCar = ({ values, setValues, status, setStatus }) => {
  ...
  useEffect(() => {
    if ((status === 'req' || status === 'res') && values.license_number.length >= 7) {
  ...
  const doApi = async (numLicense) => {
    try {
      ...
      console.log('err');
      loadingMsg("הבקשה נכשלה. יש לוודא כי הוזן מס' רישוי נכון.",'red','error')
      setStatus('ready');
    }
  };
  ...
  const validationLicenseNumber = (e) => {
    ...
    if (/^[0-9]{7}$/.test(value)) {
      const formattedValue = `${value.substring(0, 2)}-${value.substring(2, 5)}-${value.substring(5)}`;
      e.target.value = formattedValue;
      setStatus('ready');
    } else if (/^[0-9]{8}$/.test(value)) {
      const formattedValue = `${value.substring(0, 3)}-${value.substring(3, 5)}-${value.substring(5)}`;
      e.target.value = formattedValue;
      setStatus('ready');
    } else {
      setStatus('err');
  ...
  return (
    <>
      ...
      <div className='flex text-sm h-4 text-red-400 -mt-4 pr-4'>{status === 'err' && <p>מס' רישוי לא תקין</p>}</div>
    </>
  );
};
